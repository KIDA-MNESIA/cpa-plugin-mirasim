...
func (c *oauthCoordinator) startPage(state string) pluginapi.ManagementResponse {
	...
	data := startPageData{AuthURL: session.authURL, CallbackURL: session.callbackURL, Field: pastedCallbackField, Minutes: int(oauthLoginTTL.Minutes())}
	c.mu.Unlock()
...
// The form action is relative so that it resolves against the address this
// page was opened on, including any path prefix a reverse proxy adds in front
// of CPA.
var startPageTemplate = template.Must(template.New("start").Parse(`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">` +
	...
	`.button,button{display:inline-block;background:#171717;color:#fff;border:0;border-radius:6px;padding:.6rem 1rem;font:inherit;text-decoration:none;cursor:pointer}` +
	...
	`<li><p><a class="button" href="{{.AuthURL}}" target="_blank" rel="noopener noreferrer">前往 Mirasim 授权</a></p>` +
	`<p class="en">Authorize with Mirasim. It opens in a new tab.</p></li>` +
